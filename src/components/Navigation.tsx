          ...
          <div className="hidden lg:flex items-center space-x-4 xl:space-x-6">
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`font-inter font-medium text-sm xl:text-base text-navy-900 hover:text-gold-500 transition-colors duration-300 relative whitespace-nowrap ${
            ...
            <div className="py-4 sm:py-6 space-y-2 sm:space-y-4 px-4 sm:px-6">
              {navItems.map((item) => (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`block py-2 font-inter font-medium text-navy-900 hover:text-gold-500 transition-colors duration-300 relative ${
                    location.pathname === item.path ? 'text-gold-500' : ''
                  }`}
